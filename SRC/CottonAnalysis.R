...
cotton_data$in_world_cup <- as.numeric(sapply(cotton_data$DATE, function(date) is_in_world_cup_window(date, world_cup_dates)))

# Create lagged variables for cotton prices
cotton_data <- cotton_data %>%
  arrange(DATE) %>%
  mutate(
    lag1 = lag(PCOTTINDUSDM),
    lag2 = lag(PCOTTINDUSDM, 2),
    price_change = PCOTTINDUSDM - lag(PCOTTINDUSDM),
    percent_change = calculate_percent_change(PCOTTINDUSDM),  # Calculate percent changes
    in_world_cup = lag(in_world_cup, 1)
  ) %>%
  filter(!is.na(lag2))  # Remove rows with missing values in lag2

# Check for perfect multicollinearity
cor_matrix <- cor(cotton_data[, c("PCOTTINDUSDM", "lag1", "lag2", "price_change", "percent_change", "in_world_cup")], use = "complete.obs")
print(cor_matrix)

# Calculate average percent changes for World Cup and non-World Cup periods
percent_changes <- cotton_data %>%
...
ggplot(percent_changes, aes(x = factor(in_world_cup), y = avg_percent_change, fill = factor(in_world_cup))) +
  ...
  ylab("Average Cotton Percent Change") +
  ggtitle("Average Cotton Percent Changes in World Cup vs. Non-World Cup Periods") +
...
cotton_in_world_cup <- cotton_data %>% filter(in_world_cup == 1)
cotton_outside_world_cup <- cotton_data %>% filter(in_world_cup == 0)

# Compare percent changes between World Cup and non-World Cup periods
t_test_result <- t.test(cotton_in_world_cup$percent_change, cotton_outside_world_cup$percent_change)

# Display the t-test result
print(t_test_result)

# Visualize percent changes over time
ggplot(cotton_data, aes(x = DATE, y = percent_change, color = factor(in_world_cup))) +
  geom_line() +
  xlab("Date") +
  ylab("Cotton Percent Change") +
  ggtitle("Cotton Percent Changes Over Time") +
  scale_color_manual(name = "World Cup", values = c("0" = "blue", "1" = "red"))

# Econometric Modeling
econometric_model <- lm(PCOTTINDUSDM ~ in_world_cup + lag1 + lag2, data = cotton_data)
summary(econometric_model)

# Create a time series object
cotton_ts <- ts(cotton_data[, c("PCOTTINDUSDM", "in_world_cup")], start = min(cotton_data$DATE), frequency = 1)

# Granger Causality Test
granger_test <- grangertest(cotton_ts, order = 2)

# Display the Granger causality test result
print(granger_test)

# ARIMA Modeling
cotton_diff <- diff(cotton_data$PCOTTINDUSDM)

# ACF and PACF plots for ARIMA parameter selection
acf(cotton_diff, lag.max = 20)
pacf(cotton_diff, lag.max = 20)

# Identify ARIMA parameters
arima_order <- c(2, 1, 2)  # Replace with the identified order from the plots

# Fit ARIMA model
arima_model <- arima(cotton_data$PCOTTINDUSDM, order = arima_order)
summary(arima_model)
...
# VAR Modeling
var_order <- VARselect(cotton_data[, c("PCOTTINDUSDM", "in_world_cup")], lag.max = 10)
selected_lag <- var_order$selection[1]  # Choose the lag order based on the selection criteria

# Fit VAR model
var_model <- VAR(cotton_data[, c("PCOTTINDUSDM", "in_world_cup")], p = selected_lag)
summary(var_model)
...
irf <- irf(var_model, impulse = "in_world_cup", response = c("PCOTTINDUSDM", "in_world_cup"), n.ahead = 10)
plot(irf)
